...
/* debug option */
/* -------------------------------------------------------------------- */
#define SENSOR_SKY_MAG_DBG_ENABLE
#ifdef SENSOR_SKY_MAG_DBG_ENABLE
...
#define PANTECH_AVOID_DEADLOCK //P11771. 110819 for PRESTO
...
geomagnetic_i2c_open(void)
{
	...
	printk("## %s ##\n", __func__);
	rc = gpio_tlmm_config(GPIO_CFG(170, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc) {
		pr_err("%s: Could not configure gpio 170\n", __func__);
		return rc;
	}
	rc = gpio_tlmm_config(GPIO_CFG(171, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc) {
		pr_err("%s: Could not configure gpio 171\n", __func__);
		return rc;
	}
#endif
    return 0;
...
geomagnetic_i2c_close(void)
{
	...
	rc = gpio_tlmm_config(GPIO_CFG(170, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc) {
		pr_err("%s: Could not configure gpio 170\n", __func__);
		return rc;
	}
	rc = gpio_tlmm_config(GPIO_CFG(171, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc) {
		pr_err("%s: Could not configure gpio 171\n", __func__);
		return rc;
	}
