...
#ifdef CONFIG_SENSOR_POWER_CONTROL
#include <mach/vreg.h>
...
/* debug option */
/* -------------------------------------------------------------------- */
#define SENSOR_SKY_ACC_DBG_ENABLE
#ifdef SENSOR_SKY_ACC_DBG_ENABLE
#define dbg(fmt, args...)   printk("[SENSOR_MAG] " fmt, ##args)
#else
#define dbg(fmt, args...)
#endif
...
#ifdef CONFIG_SENSOR_POWER_CONTROL	
    struct regulator *vdd_l10;
    struct regulator *vdd_lvs1;
#endif	
    int suspend;
    int suspend_enable;
};
...
{
	dbg("%s \n", __func__);
#if 0 //def CONFIG_SENSOR_POWER_CONTROL //LS5_P11771_20110926_Current Optimization
	int rc;

	rc = gpio_tlmm_config(GPIO_CFG(124, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc) {
		pr_err("%s: Could not configure gpio 124\n", __func__);
		return rc;
	}
	rc = gpio_tlmm_config(GPIO_CFG(127, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc) {
		pr_err("%s: Could not configure gpio 127\n", __func__);
		return rc;
	}
#endif
...
static int yas_acc_i2c_close(void)
{
	...
	rc = gpio_tlmm_config(GPIO_CFG(124, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc) {
		pr_err("%s: Could not configure gpio 124\n", __func__);
		return rc;
	}
	rc = gpio_tlmm_config(GPIO_CFG(127, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc) {
		pr_err("%s: Could not configure gpio 127\n", __func__);
		return rc;
	}
...
#ifdef CONFIG_SENSOR_POWER_CONTROL
    data->vdd_l10 = regulator_get(NULL, "8058_l10");
    if(IS_ERR(data->vdd_l10)) {
	    	rc = PTR_ERR(data->vdd_l10);
	    	printk(KERN_ERR "<L10> %s: regulator get of failed (%d)\n", __func__, rc);
	    	err = -EINVAL;
        	goto ERR1;
    }

    rc = regulator_set_voltage(data->vdd_l10, 3000000, 3000000);
	if (rc) {
		printk(KERN_ERR "<L10> %s: vreg set level failed (%d)\n", __func__, rc);
	}
	
	rc = regulator_enable(data->vdd_l10);
	if (rc) {
		printk(KERN_ERR "<L10> %s: vreg enable failed (%d)\n",__func__, rc);
		regulator_put(data->vdd_l10);
		err = -EINVAL;
        	goto ERR1;
	}

	data->vdd_lvs1= regulator_get(NULL, "8058_lvs1");
    if(IS_ERR(data->vdd_lvs1)) {
	    	rc = PTR_ERR(data->vdd_lvs1);
	    	printk(KERN_ERR "<LVS1> %s: regulator get of failed (%d)\n", __func__, rc);
	    	err = -EINVAL;
        	goto ERR1;
    }

	rc = regulator_enable(data->vdd_lvs1);
	if (rc) {
		printk(KERN_ERR "<LVS1> %s: vreg enable failed (%d)\n",__func__, rc);
		regulator_put(data->vdd_lvs1);
		err = -EINVAL;
        	goto ERR1;
	}
...
static int yas_acc_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	dbg("%s suspend = %d, suspend_enable = %d \n", __func__, data->suspend, data->suspend_enable );
...
#ifdef CONFIG_SENSOR_POWER_CONTROL
	dbg("%s \n", __func__);
	err=regulator_is_enabled(data->vdd_l10);
	if(err>0) {
		err = regulator_disable(data->vdd_l10);
		dbg("%s vdd_l10 err = %d \n", __func__, err );
	}

	err=regulator_is_enabled(data->vdd_lvs1);
	if(err>0) {
		err = regulator_disable(data->vdd_lvs1);
		dbg("%s vdd_lvsl err = %d \n", __func__, err );
	}
#endif
...
static int yas_acc_resume(struct i2c_client *client)
{
	...
	err=regulator_is_enabled(data->vdd_l10);
	if(err<=0) {
		err = regulator_enable(data->vdd_l10);
		dbg("%s vdd_l10 err = %d \n", __func__, err );
	}

	err=regulator_is_enabled(data->vdd_lvs1);
	if(err<=0) {
		err = regulator_enable(data->vdd_lvs1);
		dbg("%s vdd_l10 err = %d \n", __func__, err );
	}
#endif
    mutex_lock(&data->data_mutex);
