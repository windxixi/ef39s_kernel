...
#define show_one_perflock(file_name)                  \
static ssize_t show_##file_name                       \
(struct cpufreq_policy *policy, char *buf)            \
{                                                     \
  return cpufreq_perflock_show_##file_name (policy, buf);  \
}

show_one_perflock(perf_lock);
show_one_perflock(perf_unlock);
...
#define store_one_perflock(file_name)                           \
static ssize_t store_##file_name                                \
(struct cpufreq_policy *policy, const char *buf, size_t count)  \
{                                                               \
  return cpufreq_perflock_store_##file_name (policy, buf, count);    \
}

store_one_perflock(perf_lock);
store_one_perflock(perf_unlock);
...
int cpufreq_perflock_set_governor(struct cpufreq_policy* policy, 
                                  const char* buf)
{
  return store_scaling_governor(policy, buf, strlen(buf));
}
...
/*jwjeon100717@DS2*/
#ifdef CONFIG_CPU_FREQ_PERFLOCK_USERSPACE
define_one_rw0666(perf_lock);
define_one_rw0666(perf_unlock);
#endif
/**
