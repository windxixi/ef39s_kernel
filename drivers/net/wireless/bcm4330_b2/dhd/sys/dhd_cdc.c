...
dhd_prot_ioctl(dhd_pub_t *dhd, int ifidx, wl_ioctl_t * ioc, void * buf, int len)
{
	...
	if ((!ret) && (ioc->cmd == WLC_SET_VAR) && (!strcmp(buf, "wme_dp"))) {
		...
		dhd->wme_dp = (uint8) ltoh32(val);
				...
				dhd->mkeep_alive.keep_alive_id = ppkt->keep_alive_id;
                dhd->mkeep_alive.period_msec = ppkt->period_msec;
				dhd->mkeep_alive.version = WL_MKEEP_ALIVE_VERSION;
				dhd->mkeep_alive.length = WL_MKEEP_ALIVE_FIXED_LEN;
        }
#endif
