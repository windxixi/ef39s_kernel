        ...
        char *ext)
{
	...
	if (wrqu->data.length != 0) {
		...
		if (strncmp(sub_cmd, "AP_SET_MAC_FLTR", strlen("AP_SET_MAC_FLTR"))) {
			WL_ERROR(("ERROR: sub_cmd: %s != 'AP_SET_MAC_FLTR'!\n", sub_cmd));
			goto exit_proc;
		}
		...
		set_ap_mac_list(dev, (char*)&mflist_set);
...
#ifdef CUSTOMER_HW_PT
	else if (strnicmp(ap->sec, "mixed-psk", strlen("mixed-psk")) == 0) {
...
set_ap_mac_list(struct net_device *dev, char *buf)
{
	...
	if (mac_mode == MACLIST_MODE_DISABLED) {
		...
		char mac_buf[256] = {0};
		struct maclist *assoc_maclist = (struct maclist *) mac_buf;
		bool deny_if_matched = (mac_mode == MACLIST_MODE_ENABLED);
		...
		if (assoc_maclist->count) {
			...
			for (i = 0; i < assoc_maclist->count; i++) {
				...
				for (j = 0; j < maclist->count; j++) {
					if (!bcmp(&assoc_maclist->ea[i], &maclist->ea[j], ETHER_ADDR_LEN)) {
						if (deny_if_matched) {
							WL_SOFTAP(("black match, do deauth/disassoc \n"));
							scbval.val = htod32(1);
							bcopy(&assoc_maclist->ea[i], &scbval.ea, ETHER_ADDR_LEN);
							ioc_res = dev_wlc_ioctl(dev, WLC_SCB_DEAUTHENTICATE_FOR_REASON, &scbval, sizeof(scb_val_t));
							check_error(ioc_res, "ioctl ERROR:", __FUNCTION__, __LINE__);
						} else {
...
#ifdef CUSTOMER_HW_PT
static int
iwpriv_set_ap_80211_mode(struct net_device *dev, 
	struct iw_request_info *info,
	union iwreq_data *wrqu,
	char *ext)
{
	...
	WL_SOFTAP((">>> Got IWPRIV AP_80211_MODE IOCTL:  info->cmd:%x, \
			info->flags:%x, u.data:%p, u.len:%d\n",
			info->cmd, info->flags,
			wrqu->data.pointer, wrqu->data.length));
	...
	if (strncmp(sub_cmd, "AP_80211_MODE", strlen("AP_80211_MODE"))) {
		WL_ERROR(("ERROR: sub_cmd: %s != 'AP_80211_MODE'!\n", sub_cmd));
		goto exit;
	}

	if (get_parmeter_from_string(&str_ptr, "MODE=", PTYPE_STRING, mode, 3) != 0) {
		WL_ERROR(("ERROR: MODE param is missing\n"));
		goto exit;
	}

	if (!strncmp(mode, "bgn", strlen("bgn"))) {
		nmode = 1;
		gmode = GMODE_AUTO;
	} else if (!strncmp(mode, "bg", strlen("bg"))) {
		nmode = 0;
		gmode = GMODE_AUTO;
	} else if (!strncmp(mode, "g", strlen("g"))) {
		nmode = 0;
		gmode = GMODE_ONLY;
	} else if (!strncmp(mode, "b", strlen("b"))) {
		nmode = 0;
		gmode = GMODE_LEGACY_B;
	} else {
		WL_ERROR(("%s invalid 802.11 mode value %s\n", __FUNCTION__, mode));
		goto exit;
	}
	...
	if ((res = dev_wlc_intvar_set(dev, "mpc", 0))) {
		WL_ERROR(("%s fail to set mpc\n", __FUNCTION__));
		goto exit;
	}
	...
	if ((res = dev_wlc_ioctl(dev, WLC_SET_GMODE, &gmode, sizeof(gmode)))) {
		WL_ERROR(("%s fail to set gmode\n", __FUNCTION__));
		goto exit;
	}
	...
	if ((res = dev_wlc_intvar_set(dev, "mpc", 1))) {
		WL_ERROR(("%s fail to set mpc\n", __FUNCTION__));
		goto exit;
	}
...
#ifdef CUSTOMER_HW_PT
	NULL,
	(iw_handler)iwpriv_set_ap_80211_mode,
#endif
#endif 
#if defined(CSCAN)
